...
export default function CheckoutPage() {
    ...
    try {
      const res = await fetch(
        "https://api-burgerli.iwebtecnology.com/api/mercadopago/create-order",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            items: cart.map((product: ProductType) => ({
              title: product.name,
  ...
  return (
    <section className="w-full px-10 py-5 flex md:flex-row flex-col gap-20 md:gap-64">
        <div className="md:w-1/2 w-full">
            <h2 className={`${pattaya.className} text-xl mt-6`}>Medio de entrega</h2>
            <ul className="flex flex-col mt-3 gap-10 font-semibold w-full">
            ...
            <h3 className={`${pattaya.className} text-xl mt-6`}>Datos personales</h3>
            <div className="flex flex-col gap-5 mt-10">
                <label htmlFor="name" className="font-bold">Nombre</label>
                <input type="text" id="name" placeholder="Nombre y apellido" className="border-b-2 border-primary/30" />
                <label htmlFor="email" className="font-bold">Correo electrónico</label>
                <input type="email" id="email" placeholder="email@gmail.com" className="border-b-2 border-primary/30" />
                <label htmlFor="phone" className="font-bold">Teléfono</label>
                <input type="tel" id="phone" placeholder="123456789" className="border-b-2 border-primary/30" />
            </div>
        </div>
        <div className="md:w-1/2 w-full">
            ...
            <ul className="flex flex-col mt-3 gap-5 font-semibold w-full">
                ...
                <li className="flex items-center text-gray-500 justify-between gap-3">
                    <p>Delivery</p>
                    <p>$5.0000</p>
                </li> <li className="flex items-center text-gray-500 justify-between gap-3">
            ...
            <div onClick={handleClick} className="w-full mx-auto cursor-pointer">
                <svg width="100%" height="74" viewBox="0 0 572 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <feBlend mode="normal" in2="BackgroundImageFix" result="effect1_dropShadow_1042_8859"/>
                    <feBlend mode="normal" in="SourceGraphic" in2="effect1_dropShadow_1042_8859" result="shape"/>
                    </filter>
                    </defs>
                </svg>
            </div>
